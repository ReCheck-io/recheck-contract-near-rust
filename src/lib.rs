...
mod tests {
    ...
    fn get_context(input: Vec<String>, is_view: bool) -> VMContext {
        VMContext {
            current_account_id: "recheck-dev.testnet".parse().unwrap(),
            signer_account_id: "recheck-dev.testnet".parse().unwrap(),
            signer_account_pk: vec![0, 1, 2],
            predecessor_account_id: "recheck-dev.testnet".parse().unwrap(),
            input,
            block_index: 0,
            block_timestamp: 0,
            account_balance: 0,
            ...
            storage_usage: 0,
            attached_deposit: 0,
            prepaid_gas: near_sdk::Gas::from(10u64.pow(18)),
    ...
    fn records() {
        ...
        testing_env!(context);
        // instantiate a contract variable with default state
        let mut contract = ObjectRecord::default();
        contract.records();
        // confirm that we received correct
        assert_eq!(1, 1);
    }
}